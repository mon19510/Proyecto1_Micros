...
isr: ;interrupcion
    BANKSEL PORTB
    btfsc ((INTCON) and 07Fh), 0 ;revision de interrupciones portb
    call pushbuttons
    btfsc ((INTCON) and 07Fh), 2 ;revision overflow tmr0
    call intmr0
    btfsc ((PIR1) and 07Fh), 0 ;revision overflow tmr1
    call intmr1
...
;------------- Config Interrupciones ------;
    ...
    movlw 00000111B ;activando en ((PORTB) and 07Fh), 0 y ((PORTB) and 07Fh), 1
    movwf IOCB

    BANKSEL INTCON
    bcf ((INTCON) and 07Fh), 0
    bsf ((INTCON) and 07Fh), 7 ;global interrupt enable bit
    bsf ((INTCON) and 07Fh), 3 ;PORTB interrupt change enable bit
    bsf ((INTCON) and 07Fh), 5 ;timer0 interrupt
    bcf ((INTCON) and 07Fh), 2 ;time interrupt enable flag
